...
object CommonMethods {
    ...
    fun mergeVideoEp(context: Context, firstVideoPath: String, secondVideoPath: String, onMerged: MainActivityVM.OnMerged)
    {
        val dir = context.filesDir.absolutePath + "/AppTunix/"
    ...
    fun appendTwoVideos(firstVideoPath: String, secondVideoPath: String): String? {
        try {
            ...
            val fc: FileChannel = RandomAccessFile(
                Environment.getExternalStorageDirectory().toString() + "/mergeVideo.mp4", "rw"
            ).channel
